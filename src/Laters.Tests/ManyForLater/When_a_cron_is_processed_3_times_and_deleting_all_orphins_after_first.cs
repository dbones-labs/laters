...
class When_a_cron_is_processed_3_times_and_deleting_all_orphins_after_first
{
    ...
    {
        Exception? caught = null;
        await _sut.InScope(schedule => schedule.ForgetAboutAllOfIt<MinimalHello>("greetings"));
        
        SystemDateTime.Set(()=> _thirdSlice);
        caught = await Rig.TryWait(() => _sut.Monitor.NumberOfCallTicksFor<MinimalHello>() >= 2, TimeSpan.FromMilliseconds(500));
        await Task.Delay(50);
    ...
    It should_not_process_the_cron_twice = () =>
